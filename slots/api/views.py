...
from slots.models import AppointmentSlot, TimeSlot, PlummHealthRecurring
...
@api_view(['POST', ])
@permission_classes([])  # Allowing any user to access this endpoint, modify as needed
@authentication_classes([])  # No authentication for simplicity, add authentication as needed
def set_recurring_slot_plumm(request):
    ...
    if request.method == 'POST':
        ...
        # Get the time zone from the API data
        timezone = pytz.timezone(timezone_str)

        # Convert date strings to datetime objects with time zone information
        ...
        # Convert recurring_time string to time object
        ...
        # Generate recurring slots with the user-provided recurring time
        ...
        # Save recurring slots to the database
        # save_recurring_slots_to_database(request.user, recurring_slots)

        # Return a JSON response with the recurring dates
...
def generate_recurring_slots_with_time_and_repeat_on(start_date, end_date, repeat, repeat_every,
    ...
    while current_date <= end_date:
        ...
        # Check if we've reached the end repeat conditions
...
def is_repeating_with_repeat_on(current_date, repeat, repeat_on_list, repeat_on_month, repeat_on_day):
    # Implement logic to check if the current date satisfies the repeat conditions
    if repeat == 'Daily':
        return True  # Daily logic (always true)
        ...
        # Check if the current day, month, and year match the specified repeat_on_day, repeat_on_month, and current year
        return (
    ...
    # Default to False if repeat option is not recognized
    return False
def is_end_reached(current_date, end_repeat, end_repeat_on, end_repeat_after):
    ...
    # Default to False if end repeat option is not recognized
    return False

def save_recurring_slots_to_database(user, recurring_slots):
    # Save recurring slots to the database
    for slot_date in recurring_slots:
        recurring_slot = PlummHealthRecurring(
            user_id=user,
            start_date=slot_date,
            end_date=slot_date,  # Assuming the end_date is the same as start_date for simplicity
            repeat='Daily',  # You may want to add a 'Custom' option to the choices for clarity
            repeat_every=1,
            repeat_on=None,
            repeat_on_day=None,
            repeat_on_month=None,
            end_repeat='Never',  # Assuming 'Never' for simplicity
            end_repeat_on=None,
            end_repeat_after=None
        )
        recurring_slot.save()