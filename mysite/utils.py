...
def unique_company_id_generator(instance):
    ...
    """
    size = 9
    company_id = random_string_generator(size=size)

    Klass = instance.__class__
    qs_exists = Klass.objects.filter(company_id=company_id).exists()
    if qs_exists:
        return
    return company_id
...
def unique_appointment_id_generator(instance):
    """
    This is for a appointment_id field
    :param instance:
    :return:
    """
    size = random.randint(5, 7)
    appointment_id = "AP-" + random_string_generator(size=size, chars=string.ascii_uppercase + string.digits) + "-(A)"

    Klass = instance.__class__
    qs_exists = Klass.objects.filter(appointment_id=appointment_id).exists()
    if qs_exists:
        return None
